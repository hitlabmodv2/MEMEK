...
exports.totalcase = () => {
   var file = fs.readFileSync("../Alicia.js").toString()
...
exports.smsg = (clientx, m, store) => {
  ...
  if (m.key) {
    ...
    m.sender = clientx.decodeJid(m.fromMe && clientx.user.id || m.participant || m.key.participant || m.chat || '');
    if (m.isGroup) m.participant = clientx.decodeJid(m.key.participant) || '';
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.sender = clientx.decodeJid(m.msg.contextInfo.participant);
      m.quoted.fromMe = m.quoted.sender === clientx.decodeJid(clientx.user.id);
      ...
      m.quoted.delete = () => clientx.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => clientx.copyNForward(jid, vM, forceForward, options);
      m.quoted.download = () => clientx.downloadMediaMessage(m.quoted);
    }
  }
  if (m.msg.urls) m.download = () => clientx.downloadMediaMessage(m.msg);
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? clientx.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : clientx.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => clientx.copyNForward(jid, m, forceForward, options);
  clientx.appenTextMessage = async (text, chatUpdate) => {
    let messages = await generateWAMessage(m.chat, {
      text: text,
      mentions: m.mentionedJid
    }, {
      userJid: clientx.user.id,
      quoted: m.quoted && m.quoted.fakeObj
    });
    messages.key.fromMe = areJidsSameUser(m.sender, clientx.user.id);
    ...
    clientx.ev.emit('messages.upsert', msg);
  };
  return m;
};
...
exports.GIFBufferToVideoBuffer = async (image) => {
    ...
    var buffer5 = await fs.readFileSync(`./Media/trash/${filename}.mp4`)
    Promise.all([unlink(`./Media/trash/${filename}.mp4`), unlink(`./Media/trash/${filename}.gif`)])
    return buffer5
}