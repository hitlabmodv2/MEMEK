...
async function pagi(nova, x, data) {
    ...
    return await nova.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(nova, x, data) {
    let row = [];
    let ment = [];
...
async function malam(nova, x, data) {
...
async function skill(nova, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await nova.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, nova, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await nova.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(nova, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(nova, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(nova, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(nova, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
        } else {
            await skill(nova, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, nova, data);
}

async function run_vote(nova, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
...
async function run_malam(nova, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
...
async function run_pagi(nova, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nova, data);
            break;
