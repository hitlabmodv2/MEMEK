...
async function clearConsole() {
    ...
    function createTmpFolder() {
        ...
        if (!fs.existsSync(folderPath)) {
            ...
            console.log(chalk.blue.bold(`[ Success ] Folder '${folderName}' berhasil dibuat.`));
        }
    }
    createTmpFolder();
...
async function AliceConnect() {
    ...
    const phoneNumber = await question("Enter Your Number Active: ")
    const code = await Alice.requestPairingCode(phoneNumber, 'ALICEV10')
    console.log(`${chalk.blue.bold('✅ Pairing Codes')} : ${chalk.blue.bold(code)}`);
}
